    ...
    async def execute_tool(
        ...
        *,
        ...
        if run_context is not None:
            ...
            # Desktop fork: ``requires_confirmation`` was computed and then
            # ignored — a write-class tool in DEFAULT mode fell straight
            # through and executed. When the host supplies a
            # ``permission_callback`` (the desktop UI does, per chat turn),
            # ask it and honour a denial. No callback ⇒ previous behaviour.
            # Keep this block when merging upstream — see
            # backend/tests/desktop/test_permission_callback_invoked.py.
            if decision.requires_confirmation:
                perm_cb = getattr(run_context, "permission_callback", None)
                if perm_cb is not None:
                    verdict = perm_cb({
                        # Several key spellings: the desktop UI and tests read
                        # different ones, and a payload mismatch silently looks
                        # like "the callback never fired".
                        "tool": tool_name,
                        "name": tool_name,
                        "tool_name": tool_name,
                        "args": args,
                    ...
                    if str(verdict).strip().lower() in (
                        "deny", "denied", "no", "reject", "rejected", "false",
                    ):
                        return ToolResult(
                            success=False,
                            output="",
