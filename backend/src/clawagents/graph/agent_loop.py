...
# ─── Desktop fork: per-run options upstream does not carry ───────────────
#
# The desktop gateway runs many chats in one process, so it needs to say
# *which* chat a run belongs to (``session_id`` / ``session_dir``) and how to
# reach the user for a permission prompt (``permission_callback``). Upstream's
# ``AgentRunConfig`` has no home for these, and adding fields there would fork
# a file that is otherwise byte-identical to upstream and merges cleanly.
#
# Instead they ride on the ``RunContext``, which every run already has and
# which the bootstrapper adopts verbatim when the caller supplies one. Only two
# places read them back: ``RunBootstrapper._init_persistence`` (SessionWriter)
# and ``tools/registry.py`` (permission_callback). Keep this helper and those
# two readers when merging upstream — everything else stays untouched.
DESKTOP_SESSION_ID_KEY = "desktop_session_id"
DESKTOP_SESSION_DIR_KEY = "desktop_session_dir"


def _apply_desktop_run_options(
    run_context: Optional[RunContext],
    user_context: Any,
    session_id: Optional[str],
    session_dir: Optional[Any],
    permission_callback: Optional[Callable[[dict], Any]],
) -> Optional[RunContext]:
    """Attach desktop-only per-run options to the run context.

    Returns the context to hand to :class:`AgentRunConfig`. When the caller
    passed none of the desktop options and no context, returns ``None`` so the
    bootstrapper builds one exactly as upstream does. Idempotent: calling it
    twice with the same values changes nothing, which lets both the public
    wrapper and the core apply it without ordering concerns.
    """
    if session_id is None and session_dir is None and permission_callback is None:
        return run_context
    ...
    if not isinstance(getattr(run_context, "_metadata", None), dict):
        run_context._metadata = {}
    if session_id is not None:
        run_context._metadata[DESKTOP_SESSION_ID_KEY] = session_id
        # The provider session id (prompt-cache key / session affinity) should
        # be the chat's identity too, not a fresh uuid per turn.
        run_context.session_id = session_id
    if session_dir is not None:
        run_context._metadata[DESKTOP_SESSION_DIR_KEY] = session_dir
    return run_context
...
async def run_agent_graph(
    ...
    # ── Desktop fork: see _apply_desktop_run_options above ──
    ...
    run_context = _apply_desktop_run_options(
        run_context, user_context, session_id, session_dir, permission_callback
    )
...
async def _run_agent_graph_core(
    ...
    file_blocks: Optional[list[dict]] = None,
    session_end_tail: bool = True,
    # ── Desktop fork: accepted here too so callers that reach the core
    # directly (tests, embedders) behave identically to the public wrapper.
    # Re-applying is idempotent when the wrapper already did it.
    session_id: Optional[str] = None,
    session_dir: Optional[Any] = None,
    permission_callback: Optional[Callable[[dict], Any]] = None,
) -> AgentState:
