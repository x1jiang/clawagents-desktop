...
def _extract_plan_contract(plan_text: str) -> tuple[list[str], list[str]]:
    invariants: list[str] = []
    commands: list[str] = []
    section = ""
    fenced = False
    ...
    for raw in plan_text.splitlines():
        ...
        if heading and not fenced:
            ...
            if _VERIFICATION_HEADING.search(title):
    ...
    if not invariants:
        for raw in plan_text.splitlines():
            ...
            if re.search(r"\b(?:must|never|only after|before)\b", text, re.IGNORECASE):
                invariants.append(text)
    return invariants[:24], commands[:24]
...
def approve_plan_contract(
    ...
    if not body:
        ...
        return None
    invariants, commands = _extract_plan_contract(body)
    ...
    if not invariants and not commands:
        clear_contract(run_context, workspace)
        return None
...
def is_high_impact_command(command: str) -> bool:
    ...
    if not normalized or _SAFE_EXTERNAL_MARKERS.search(normalized):
        return False
...
def gate_tool_call(tool_name: str, args: dict[str, Any], run_context: Any) -> str | None:
    ...
    if contract.get("authorization_consumed"):
        return (
            ...
            "was consumed by a successful high-impact action. Re-run every verification "
            "gate before another publish/deploy action."
        )
...
def observe_tool_result(
    ...
    *,
    ...
    if tool_name == "execute" and is_high_impact_command(command):
        ...
        _write_contract(contract, run_context)
        return "[plan invariant gate: verification authorization consumed]"

    planned = contract.get("verification_commands") or []
    if tool_name == "execute" and command in planned:
